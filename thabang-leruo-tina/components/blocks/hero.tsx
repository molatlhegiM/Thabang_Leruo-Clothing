...
const ImageBlock = ({ image }: { image: PageBlocksHeroImage }) => {
  if (image.videoUrl) {
    ...
    if (idx !== -1) videoId = image.videoUrl.substring(idx + embedPrefix.length).split('?')[0];
    const thumbnailSrc = image.src
      ...
      : '';
    return (
      <HeroVideoDialog videoSrc={image.videoUrl} thumbnailSrc={thumbnailSrc} thumbnailAlt='Hero Video' />
    );
  }
  ...
  fields: [
    ...
    },
    {
      ...
      fields: [
        ...
        { name: 'videoUrl', label: 'Video URL', type: 'string', description: 'Embed version of YouTube video' },
      ],
    },
  ],
};
